...
States3y <- States3y %>%
  mutate(npl_per_capita = net_pension_liability / population,
         employee_debt = net_pension_liability + net_opeb_liability,
...
SD3y <- merge(SD3y_v2, SD3y_v1[, c("state.abb", "name", "year", "total_assets")], 
               by = c("state.abb", "name", "year"), all.x = TRUE)


SD3y <- SD3y %>%
  mutate(npl_per_student= net_pension_liability / students,
         employee_debt = net_pension_liability + net_opeb_liability,
         employee_debt_per_capita = (net_pension_liability + net_opeb_liability)/ students,
         budget_deficit = revenues - expenses) 
...
### Net Budget States
Negative value indicates a budget deficit, while a positive value indicates a budget surplus. 

```{r, echo=FALSE}

States3y_deficit_sort <- States3y %>%
  filter(year == 2022) %>%
  select(name, budget_deficit, revenues, expenses)%>%
  arrange(budget_deficit)

States3y_deficit_reactable <- reactable(
  States3y_deficit_sort,
  columns = list(
    name = colDef(
      name = "State",
      cell = function(value) tools::toTitleCase(as.character(value))),  
    budget_deficit = colDef(
      name = "2022 Net Budget",
      cell = function(value) sprintf("$%s", format(value, big.mark = ","))),
    revenues = colDef(
      name = "Revenues",
      cell = function(value) sprintf("$%s", format(value, big.mark = ","))),
...
Cities3y_deficit_sort <- Cities3y %>%
  filter(year == 2022) %>%
  select(name, budget_deficit, revenues, expenses)%>%
  arrange(budget_deficit)

Cities3y_deficit_reactable <- reactable(
  Cities3y_deficit_sort,
  columns = list(
    name = colDef(
      name = "City",
      cell = function(value) tools::toTitleCase(as.character(value))),  
...
Counties3y_deficit_sort <- Counties3y %>%
  filter(year == 2022) %>%
  select(name, budget_deficit, revenues, expenses)%>%
  arrange(budget_deficit)

Counties3y_deficit_reactable <- reactable(
  Counties3y_deficit_sort,
  columns = list(
    name = colDef(
      name = "County",
      cell = function(value) tools::toTitleCase(as.character(value))),  
...
Counties3y_deficit_reactable

```

### Net Budget School Districts
Negative value indicates a budget deficit, while a positive value indicates a budget surplus. 

```{r, echo=FALSE}

SD3y_deficit_sort <- SD3y %>%
  filter(year == 2022) %>%
  select(name, budget_deficit, revenues, expenses)%>%
  arrange(budget_deficit)

SD3y_deficit_reactable <- reactable(
  Counties3y_deficit_sort,
  columns = list(
    name = colDef(
